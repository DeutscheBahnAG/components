...
interface LogoComponent
  extends React.ForwardRefExoticComponent<LogoProps & React.RefAttributes<LogoWrapperRef>> {
  sizes: typeof LogoSizes;
  variants: typeof LogoVariants;
}

// @ts-expect-error the type resulting from React.forwardRef does not seem to support additional attributes
  ...
  (
    ...
    return (
      <Wrapper
        ref={ref}
        href={href ?? undefined}
        className={clsx('db-logo', `db-logo--size-${size}`, `db-logo--${variant}`, className)}
      >
